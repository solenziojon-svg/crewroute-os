"""
photo_audit_agent.py — Sprint 2: Photo Auto-Auditing (Empire Standard v3)
────────────────────────────────────────────────────────────────────────────
Analyzes job completion photos using Claude Vision.
Includes Safe Mode awareness in the combined entry point.
...
Return ONLY valid JSON:
{
  ...
  "quality_notes": "1-2 sentence description",
  ...
  "gate_note": "Safety or access observation if flagged",
  "client_caption": "One professional sentence for the client invoice photo"
}
""".strip()
...
class PhotoAuditAgent:
    ...
    async def analyze(
        ...
        if audit_data is None:
            audit_data = {
                ...
                "quality_notes":  "Unable to process photo automatically.",
                "quality_score":  5,
                ...
                "gate_flag":      False,
                "gate_note":      "",
                "client_caption": f"{job_type} completed — {date}.",
            }
    ...
    async def _run_vision(self, img_b64, media_type, job_id, client_name, job_type, date):
        import anthropic
    ...
    def _fetch_image_as_base64(self, url: str):
        req = urllib.request.Request(url, headers={"User-Agent": "CrewRoute-OS/1.0"})
        with urllib.request.urlopen(req, timeout=12) as resp:
            raw = resp.read()
            media_type = resp.headers.get("Content-Type", "image/jpeg").split(";")[0].strip()
            media_type = {"image/jpg": "image/jpeg", "image/png": "image/png"}.get(media_type, "image/jpeg")
            return base64.b64encode(raw).decode("utf-8"), media_type

    def _persist(self, result: PhotoAuditResult, date: str):
    ...
    async def _fire_webhook(self, result: PhotoAuditResult):
...
# ════════════════════════════════════════════════════════════════
# COMBINED ENTRY POINT (with Safe Mode check)
# ════════════════════════════════════════════════════════════════
...
async def process_job_completion(
    ...
    """
    Unified Voice + Photo entry point with Safe Mode awareness.
    """
    ...
    if safe_mode_active:
        ...
        has_photo = False  # Skip expensive vision calls

    logger.info("process_job_completion_started", has_voice=has_voice, has_photo=has_photo, safe_mode=safe_mode_active)
    ...
    if not any_success:
        return {"success": False, "error": "All agents failed", "safe_mode_active": safe_mode_active}
...
async def _test():
    ...
    result = await agent.analyze(
        job_id="TEST-001",
        photo_url="https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Biome_grassland_LLNL.jpg/320px-Biome_grassland_LLNL.jpg",
        client_name="Test Client",
        dry_run=True,
    )
    logger.info("photo_audit_test_complete", quality=result.quality_status)
